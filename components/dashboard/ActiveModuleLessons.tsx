...
export function ActiveModuleLessons({ activeCourse, completedLessonIds }: ActiveModuleLessonsProps) {
  ...
  const activeModule = (() => {
    ...
    return activeCourse.modules[activeCourse.modules.length - 1] ?? null;
  })();
    ...
    <div className="rounded-2xl border border-border bg-muted overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            {activeModule?.lessons.filter((l) => completedLessonIds.includes(l._id)).length ?? 0}/{activeModule?.lessons.length ?? 0} done
          </span>
        </div>
      </div>
      ...
      <div className="divide-y divide-border">
        {(activeModule?.lessons ?? []).map((lesson, index) => {
          ...
          const isNext =
            !isCompleted &&
            (activeModule?.lessons ?? []).slice(0, index).every((l) => completedLessonIds.includes(l._id));

          return (
            <a
              key={lesson._id}
              href={`/lessons/${lesson.slug}`}
              className={`flex items-center gap-4 px-5 py-3.5 transition-all hover:bg-background/50 group ${isNext ? "bg-primary/5" : ""}`}
            >
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold transition-all ${
                  isCompleted
                    ...
                    : isNext
                      ? "bg-primary text-background shadow-lg shadow-primary/30 animate-pulse"
                      : "bg-muted-foreground/10 text-muted-foreground border border-border"
                }`}
              >
                {isCompleted ? "✓" : isNext ? "▶" : index + 1}
              </div>
